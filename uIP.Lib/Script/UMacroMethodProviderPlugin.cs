...
namespace uIP.Lib.Script
{
    ...
    ///     - GetDicKeyStrOne{MethodName}MethodSettings: 用來取得 UMacro 的內部設定 <see cref="fpGetMacroMethodSettings"/>
    ...
    ///   - m_MacroControls: 針對 macro 的 method 的 ioctl, 若是要儲存的參數 CanStore 要設定為 true; 禁用 Dictionary輸出; 對於 GET 若是不支援請回傳 null, status設定為 true, 為相容於儲存參數, 真正使用請一併檢查回傳是否不為null ;SET則可以回傳false
    /// </summary>
    public abstract class UMacroMethodProviderPlugin
    {
        ...
        public const string PredefMacroIoctl_ScriptBeginExecNotify = "ScriptBeginExec";
        ...
        protected Dictionary<string, PluginClassProvideFunc> m_PluginClassProvideFuncs = new Dictionary<string, PluginClassProvideFunc>();
        ...
        protected Dictionary<string, Action<UScript, int, UMacro>> m_macroMethodScriptNotifyBeginExec = new Dictionary<string, Action<UScript, int, UMacro>>();
        ...
        /// <summary>
        /// Plugin class provide functions
        /// </summary>
        public Dictionary<string, PluginClassProvideFunc> PluginClassFunctions => m_PluginClassProvideFuncs;
        ...
        public Dictionary<string, fpPopupMacroConfigDialog> MacroPopupUiList => m_macroMethodConfigPopup;
        public Dictionary<string, fpGetMacroConfig> MacroGenControlList => m_macroMethodConfigFormGet;
        ...
        public UMacroMethodProviderPlugin()
        {
            ...
            m_MacroControls.Add( PredefMacroIoctl_ScriptBeginExecNotify, new UScriptControlCarrierMacro( PredefMacroIoctl_ScriptBeginExecNotify, false, true, false,
                new UDataCarrierTypeDescription[]
                {
                    new UDataCarrierTypeDescription(typeof(UScript), "Script instance"),
                    new UDataCarrierTypeDescription(typeof(int), "begin index")
                }, null, MacroIoctl_SetNotifyScriptBegExec ) );
        ...
        public bool MethodAbleConfigByPopup(string name)
        {
            return m_macroMethodConfigPopup?.ContainsKey( name ) ?? false;
        }
        public bool MethodAbleConfigByControl(string name)
        {
            return m_macroMethodConfigFormGet?.ContainsKey( name ) ?? false;
        }
        ...
        {
            bRetStatus = false;
            if ( whichMacro == null ) // invalid op
                return null;
            if ( !m_macroMethodSettingsGet.ContainsKey( whichMacro.MethodName ) ) // not contain
            {
                bRetStatus = true;
                return null;
            }
            object repo = null;
            Type t = null;
            bRetStatus = m_macroMethodSettingsGet[ whichMacro.MethodName ]?.Invoke( whichMacro, out repo, out t ) ?? true;
        ...
        private bool MacroIoctl_SetMacroMethodSettings( UScriptControlCarrier carrier, UMacro whichMacro, UDataCarrier[] data )
        {
            if ( whichMacro == null ) // invalid op
                return false;
            if ( !m_macroMethodSettingsSet.ContainsKey( whichMacro.MethodName ) )
                return true;
            if ( data == null || data.Length <= 0 )
                return false;

            return m_macroMethodSettingsSet[ whichMacro.MethodName ]?.Invoke( whichMacro, data[0].Data ) ?? true;
        }

        private bool MacroIoctl_SetNotifyScriptBegExec( UScriptControlCarrier carrier, UMacro whichMacro, UDataCarrier[] data )
        {
            if ( whichMacro == null )
                return false;
            if ( !m_macroMethodScriptNotifyBeginExec.ContainsKey( whichMacro.MethodName ) )
                return true;
            if ( data == null || data.Length < 2 )
                return false;

            if ( !UDataCarrier.GetByIndex<UScript>( data, 0, null, out var script ) || !UDataCarrier.GetByIndex( data, 1, -1, out var begIndex ) )
                return false;

            m_macroMethodScriptNotifyBeginExec[ whichMacro.MethodName ]?.Invoke( script, begIndex, whichMacro );
            return true;
        }
        ...
        /// <summary>
        /// GetDicKeyStrOne the setting of given "id" name in m_PluginClassControls list.
        /// </summary>
        ...
        protected virtual UMacro NewMacroInstance(UMacro reference)
        {
            return new UMacro( 
            ...
            for ( int i = 0 ; i < m_UserQueryOpenedMethods.Count ; i++ )
            {
                ...
                if ( m_UserQueryOpenedMethods[ i ].MethodName == strMethodNm )
                {
                    ...
                    if ( UDataCarrier.TypesCheck( immutableOnes, tmp.ImmutableParamTypeDesc ) )
                    {
                        ...
                        //};
                        ret = NewMacroInstance( tmp );
                        ret.GivenNameOfOwnerPluginClass = m_strInternalGivenName;
                        ret.ParameterCarrierImmutable = immutableOnes;
                        ret.ParameterCarrierVariable = variables;
                        ret.Invisible = tmp.Invisible;
                        ret.ConfigFirst = tmp.ConfigFirst;
                        ret.ConfigDone = tmp.ConfigDone;
        ...
        /// <summary>
        /// GetDicKeyStrOne data from macro of a script.
        ...
        /// <summary>
        /// GetDicKeyStrOne specified name "id" reporting data types.
        /// </summary>
        /// <param name="id">[id.Data]: name in string to search</param>
        /// <param name="bRetStatus">return the status. true:OK, false:NG</param>
        ...
        /// <summary>
        /// GetDicKeyStrOne the all configurable names and its types info of a opened method.
        /// </summary>
        /// <param name="macro">created macro info</param>
        ...
        protected void AutoBindingMacroPredefineCtrl( object classInst )
        {
            ...
            foreach ( var mm in m_UserQueryOpenedMethods )
            {
                ...
                // search GetDicKeyStrOne{MethodName}MethodSettings
                minfo = tp.GetMethod( $"GetDicKeyStrOne{mm.MethodName}MethodSettings", BindingFlags.Instance | BindingFlags.NonPublic | BindingFlags.DeclaredOnly );
        ...
        public bool CallClassProvideFunc( string name, out UDataCarrier ret, params UDataCarrier[] ctx )
        {
            var status = false;
            ret = null;
            if ( m_PluginClassProvideFuncs?.TryGetValue( name, out var item ) ?? false )
            {
                try
                {
                    status = item?.Call?.Invoke( out ret, ctx ) ?? false;
                }
                catch(Exception ex)
                {
                    m_fpLog?.Invoke( eLogMessageType.WARNING, 100, $"{GetType().FullName} call {name}: with exception\n{ex}" );
                }
            }
            return status;
        }
